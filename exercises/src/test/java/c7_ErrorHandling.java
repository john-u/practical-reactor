...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    /**
     * You are monitoring heart beat signal from space probe. Heart beat is sent
     * every 1 second.
     * Raise error if probe does not any emit heart beat signal longer then 3
     * seconds.
    ...
    public void houston_we_have_a_problem() {
        ...
        Flux<String> heartBeat = probeHeartBeatSignal()
                ...
                .doOnError(e -> e instanceof TimeoutException, e -> errorRef.set(e));
     ...
     * If any error occurs, exception should be further propagated as
     * `SecurityException`.
     * Keep original cause.
     */
    @Test
    ...
    public void under_the_rug() {
        Flux<String> messages = messageNode()
                // onErrorComplete not introduced until 3.5
    ...
    public void have_a_backup() {
        Flux<String> messages = messageNode()
                .onErrorResume(__ -> backupMessageNode());
    ...
    /**
     * Consume all the messages `messageNode()`, if node suddenly fails report error
     * to `errorReportService` then
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue()
                .flatMap(task -> task.execute()
                        .then(task.commit())
                        .onErrorResume(err -> task.rollback(err))
                        .thenReturn(task));
     ...
     * Using `onErrorContinue()` skip corrupted file and get the content of the
     * other files.
     */
    @Test
    ...
    /**
     * Quote from one of creators of Reactor: onErrorContinue is my billion-dollar
     * mistake. `onErrorContinue` is
     * considered as a bad practice, its unsafe and should be avoided.
     *
     * {@see <a href=
     * "https://nurkiewicz.com/2021/08/onerrorcontinue-reactor.html">onErrorContinue</a>}
     * {@see <a
     * href=
     * "https://devdojo.com/ketonemaniac/reactor-onerrorcontinue-vs-onerrorresume">onErrorContinue
     * vs
     * onErrorResume</a>} {@see <a href=
     * "https://bsideup.github.io/posts/daily_reactive/where_is_my_exception/">Where
     * is
     * my exception?</a>}
     *
     * Your task is to implement `onErrorContinue()` behaviour using
     ...
     * is cheaply made and may not return value on each read. Keep retrying until
     ...
     * You may retry to establish connection maximum of three times, so do it
     * wisely!
    ...
    /**
     * You are working with legacy system in which you need to read alerts by
     * pooling SQL table. Implement polling
    ...
    public void good_old_polling() {
        Flux<String> alerts = nodeAlerts()
                .repeatWhenEmpty(node -> node.delayElements(Duration.ofSeconds(1)))
                .repeat();
