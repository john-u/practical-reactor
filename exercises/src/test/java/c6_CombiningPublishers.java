...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary()
            .switchOnFirst((signal, flux) -> {
                if (signal.hasValue() && signal.get().metaData == "spam") {
    ...
    public void car_factory() {
        Flux<Car> producedCars = carEngineProducer()
            .zipWith(
                carChassisProducer(), 
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            if (sourceRef.get() == "A") return sourceA();
            if (sourceRef.get() == "B") return sourceB();
            return Mono.empty();
        });
    }
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(
            ...
            Function.identity(), 
            __ -> StreamingConnection.closeConnection()
        );

        StreamingConnection.startStreaming()
            .flatMapMany(Function.identity())
            .doFinally(__ -> StreamingConnection.closeConnection());
